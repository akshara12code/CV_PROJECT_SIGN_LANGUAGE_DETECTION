...
# Create white.jpg if it doesn't exist
...
class Application:
    ...
    def __init__(self):
        ...
        self.panel.place(x=100, y=50, width=480, height=400)
        ...
        self.T.config(text="Sign Language To Text Conversion", font=("Courier", 25, "bold"))

        self.panel3 = tk.Label(self.root)
        self.panel3.place(x=300, y=480)
        ...
        self.panel5.place(x=260, y=530)

        self.T3 = tk.Label(self.root)
        self.T3.place(x=10, y=530)
        ...
        self.b1.place(x=250, y=580)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=450, y=580)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=650, y=580)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=850, y=580)

        self.speak = tk.Button(self.root)
        self.speak.place(x=1100, y=530)
        self.speak.config(text="Speak", font=("Courier", 18), command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1000, y=530)
        self.clear.config(text="Clear", font=("Courier", 18), command=self.clear_fun)
    ...
    def video_loop(self):
        try:
            ...
            # Fix blue tint - convert BGR to RGB correctly
            ...
            # cvzone returns either a list of dicts or a tuple (list, img)
            hand_list = hands[0] if isinstance(hands, tuple) else hands
            if hand_list and len(hand_list) > 0:
                hand = hand_list[0]
                x, y, w, h = hand['bbox']
                ...
                if image.size > 0:
                    ...
                    if hand_list2 and len(hand_list2) > 0:
                        hand2 = hand_list2[0]
                        ...
                        self.current_image2 = Image.fromarray(white_rgb)
                        ...
                        self.b1.config(text=self.word1, font=("Courier", 16), command=self.action1)
                        self.b2.config(text=self.word2, font=("Courier", 16), command=self.action2)
                        self.b3.config(text=self.word3, font=("Courier", 16), command=self.action3)
                        self.b4.config(text=self.word4, font=("Courier", 16), command=self.action4)

            self.panel5.config(text=self.str, font=("Courier", 25), wraplength=1000)
    ...
    def predict(self, test_image):
        ...
        if ch1 == 5:
            if self.pts[4][0]>self.pts[12][0] and self.pts[4][0]>self.pts[16][0] and self.pts[4][0]>self.pts[20][0]:
                ch1 = 'Z' if self.pts[8][1]<self.pts[5][1] else 'Q'
            else:
                ch1 = 'P'
